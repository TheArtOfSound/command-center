...
# Missing GitHub-backed projects are cloned into ~/.qira-grok-ops/workspace
# automatically when their first Grok session starts.
export GROK_CONTROL_HOST="${GROK_CONTROL_HOST:-127.0.0.1}"
export GROK_CONTROL_PORT="${GROK_CONTROL_PORT:-8787}"

python -m uvicorn managed_server:app --host "$GROK_CONTROL_HOST" --port "$GROK_CONTROL_PORT" &
SERVER_PID=$!
